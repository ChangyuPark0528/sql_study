...
-- 테이블 생성과 제약조건
-- : 테이블에 부적절한 데이터가 입력되는 것을 방지하기 위해 규칙을 설정하는것
...
-- 외래 키(forein key)가 부모테이블(참조테이블)에 없다면 INSERT 불가능
INSERT INTO dept2
VALUES(10, 'gg', 3000, 100000, 'M');

INSERT INTO dept2
VALUES(20, 'gg', 1900, 100000, 'M');

UPDATE dept2
SET loca = 4000
WHERE dept2_no = 10; -- 실패(외래키 제약조건 위반)

UPDATE dept2
SET loca = 4000
WHERE dept2_no = 1900; -- 실패(외래키 제약조건 위반)

-- 제약 조건을 변경
-- 제약 조건은 추가, 삭제만 가능. 변경은 불가
-- 변경하려면 삭제 후 추가해야 합니다.
...
SELECT
    ...
    loc.street_address,
    loc.location_id
FROM members m JOIN loctions loc
