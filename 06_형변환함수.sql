...
-- NULL 변환함수 NVL2(컬럼, NULL이 아닐 경우의 값, NULL일 경우의 값)
SELECT 
    NVL2(null,'널아님','널임')
FROM dual;
...
SELECT
    DECODE('C', 'A', 'A입니다', 'B', 'B입니다.', 'C', 'C입니다.', '뭔데그게?')
FROM dual;
...
SELECT
    ...
    hire_date AS 입사일자,
    TO_CHAR((TRUNC(sysdate - hire_date) / 365),'99') AS 근속년수 -- TO_CHAR는 자를때 자동으로 반올림을 해준다.
FROM employees
WHERE TO_CHAR((TRUNC(sysdate - hire_date) / 365),'99') >= 17
ORDER BY 근속년수 DESC;
