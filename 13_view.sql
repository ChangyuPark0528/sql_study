...
GROUP BY job_title
ORDER BY AVG(salary) DESC; -- SQL구문이 짧아짐

-- 뷰 삭제
DROP VIEW view_emp;

/*
VIEW에 INSET가 일어나는 경우 실제 테이블에도 반영이 됩니다.
그래서 VIEW의 INSET, UPDATE, DELETE는 많은 제약 사항이 따릅니다.
원본 테이블이 NOT NULL인 경우에는 VIEW에 INSERT가 불가능합니다.
VIEW에서 사용하는 컬럼이 가상열인 경우에도 안됩니다.
*/

-- 두 번째 컬럼인 'full_name'은 가상열(virtual column) 이기 때문에 INSET 안됩니다.
...
-- 읽기 전용 뷰 -> WITH READ ONRY (DML 연산을 막음 -> SELECT만 허용)
CREATE OR REPLACE VIEW view_emp_test AS (
    SELECT
        employee_id,
        first_name,
