...
CREATE SEQUENCE dept2_seq
    ...
    MAXVALUE 10 -- 최대값 (기본값은 증가일때 1027, 감소일 때 -1)
    MINVALUE 1 -- 최소값 (기본값 증가일때 1, 감소일때 -1028)
    NOCACHE -- 캐시메모리 사용 여부 (CACHE) -- 미리 값을 캐시로 어느정도 저장. 개발과정에서는 잘 사용안함.
...
ALTER SEQUENCE dept2_seq INCREMENT BY -73;
...
인덱스를 생성하게되면 지정한 컬럼에 ROWID를 붙인 인덱스가 준비되고,
조회를 진행할 시 해당 인뎅스의 ROWID를 통해 빠른 스캔을 가능하게 합니다.
*/
