...
FROM info
JOIN auth -- JOIN이라고 쓰면 기본적으로 INNER JOIN
ON info.auth_id = auth.auth_id;

-- 필요한 데이터만 조회하겠다! 라고하면
-- WHRER 구문을 총해 일반 조건을 걸어주시면 됩니다.
SELECT
...
SELECT
    *
FROM info i LEFT JOIN auth a -- OUTER 키워드 생략 가능
ON i.auth_id = a.auth_id;
...
-- 좌측 테이블과 우측 테이블을 모두 읽어 중복된 데이터는 삭제되는 외부조인
SELECT
    *
FROM info i FULL JOIN auth a -- OUTER 키워드 생략 가능
ON i.auth_id = a.auth_id;
