...
EXEC my_new_job_proc('JOB3', 'test job3', 2000, 3000);

SELECT * FROM jobs;

-- job_id를 확인해서
-- 이미 존재하는 데이터라면 수정, 없다면 새롭게 추가(job_id가PK이기 때문)
...
BEGIN
    dbms_output.put_line('p_var1: ' || p_var1); -- IN: 당연히 출력
    dbms_output.put_line('p_var2: ' || p_var2); -- OUT: 값이 전달되지 않음
    dbms_output.put_line('p_var3: ' || p_var3); -- INOUT: IN의 성질을 가지고있음
    
--  p_var1 := '결과1'; IN 변수는 값 할당 자체가 불가능.
    p_var2 := '결과2';
    p_var3 := '결과3';
    
    dbms_output.put_line('END전 p_var2: ' || p_var2);
END;
...
INVALID_NUMBER -> 문자를 숫자로 변환할 때 실패한 경우
    */
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            -- WHEN으로  발생한예외가 아닌 다른예외가 발생 시 others 실행
