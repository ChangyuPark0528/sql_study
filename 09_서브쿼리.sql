...
-- 다음문장은 서브쿼리가 리턴하는 행이 여러 개라서 단일 행 연선자를 사용 할 수 없습니다.
-- 단일 행 연산자 : 주로 비교 연산자(=, >, <, >=, <= <>)를 사용하는 경우 하나의 행만 반환해야합니다.
-- 이런경우에는 다중행 연산자를 사용해야 합니다.
SELECT * FROM employees
WHERE job_id = (SELECT job_id FROM employees
...
-- SELECT 절에 서브쿼리 붙이기.
-- 스칼라 서브쿼리 라고도 칭합니다.
-- 스칼라 서브쿼리: 실행 결과가 단일값을 반환하는 서브쿼리. 주로 SELECT절이나, WHERE 절에서 사용됨.
SELECT
    e.first_name,
    d.department_name
FROM employees e
...
수정, 삭제 등이 빈번한 경우 (sql 가독성은 조인이 좀 더 뛰어납니다.)
*/
